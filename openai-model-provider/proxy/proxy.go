...
func Run(cfg *Config) error {
	...
	if handler := cfg.CustomPathHandleFuncs["GET /validate"]; handler == nil && cfg.CustomPathHandleFuncs["/validate"] == nil {
		cfg.CustomPathHandleFuncs["GET /validate"] = s.validate
	}
	...
	httpServer := &http.Server{
		Addr:    net.JoinHostPort(listenHost, cfg.ListenPort),
		Handler: mux,
	}

	fmt.Printf("[model-provider: %s] Starting OpenAI-style API proxy on %s → baseURL=%s\n", cfg.Name, httpServer.Addr, cfg.BaseURL)
